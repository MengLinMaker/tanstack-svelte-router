...
>(
  ...
  const type: 'internal' | 'external' = Solid.createMemo(() => {
    try {
      new URL(`${to}`)
      return 'external'
    } catch {}
  ...
  if (type === 'external') {
    return {
      ...propsSafeToSpread,
      ref: innerRef as Solid.ComponentProps<'a'>['ref'],
      type,
      href: to,
      ...(children && { children }),
      ...(target && { target }),
  ...
  const resolvedActiveProps: Solid.JSX.HTMLAttributes<HTMLAnchorElement> =
    isActive ? (functionalUpdate(activeProps as any, {}) ?? {}) : {}

  // Get the inactive props
  const resolvedInactiveProps: Solid.JSX.HTMLAttributes<HTMLAnchorElement> =
